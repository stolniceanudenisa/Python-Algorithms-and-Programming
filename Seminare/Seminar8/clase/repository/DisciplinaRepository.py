from seminar9.clase.repository.RepositoryException import *
...
class DisciplinaRepository:
    ...
    def adauga(self, disciplina):
        ...
        if self.gaseste_disciplina_dupa_id(disciplina.get_id()) != -1:
            raise DuplicateIDException("Exista deja o disciplina cu acest id!")
        else:
    ...
    def sterge(self, id):
        ...
        if self.gaseste_disciplina_dupa_id(id) == -1:
            raise InexistentIDException("Disciplina cu acest id nu exista!")
        else:
    ...
    def get_disciplina_by_id(self, id_disciplina):
        for i in range(0, len(self.__lista_discipline)):
            disciplina_curenta = self.__lista_discipline[i]
            if disciplina_curenta.get_id() == id_disciplina:
                return disciplina_curenta
        return -1