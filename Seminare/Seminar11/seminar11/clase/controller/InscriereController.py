...
class InscriereController:
    ...
    def studenti_inscrisi_la_disciplina(self, nume_disciplina):
        ...
        for inscriere in lista_inscrieri:
            ...
            disciplina = self.__disciplina_repository.get_by_id(id_disciplina)
    ...
    def sterge_disciplina_cascada(self, id_disciplina):
        ...
        :return:
        '''
        lista_inscrieri = self.get_all()
        i = 0
        while i < len(lista_inscrieri):
            inscriere_curenta = lista_inscrieri[i]
            if inscriere_curenta.get_disciplina_id() == id_disciplina:
                id_inscriere = inscriere_curenta.get_id()
                self.__inscriere_repository.sterge(id_inscriere)
                i = i - 1
            i = i + 1
